...
Thanks to Robert Trenary, who first introduced me to the field of
Machine Learning.
...
I Thank John Kapenga for reviewing this thesis and providing helpful feedback.
...
\begin{table}[ht]
    ...
    Normal & $\Norm$ \\
    \end{tabular}
    \caption[Abbreviations of Common Distributions]{
        \textbf{Abbreviations of Common Distributions.}
    }
...
\begin{table}[ht]
    ...
    \end{tabular}
    \caption[Summary of Exercise Dataset]{
        \textbf{Summary of Exercise Dataset.}
    }
    \label{table:dataset}
\end{table}
...
\begin{figure}[ht!]
    ...
    \textbf{Example Routine Time Series.}
    Axis are vertically ordered by $x$, $y$, $z$ (accelerometer, range from -3 to 3)
    and $q$, $r$, $s$ (gyroscope, range from -600 to 600).}
    \label{fig:routine}
\end{figure}
...
\begin{figure}[ht!]
    ...
    \caption[Slice Representation of a HMM]{
        \textbf{Slice Representation of a HMM.}
...
\begin{figure}[ht!]
    ...
    \end{tikzpicture}
    \caption[Graphical Representation of a Bayesian HMM]{
        \textbf{Graphical Representation of a Bayesian HMM.}
        Parameters $\Theta$ are not marginalized in (a) and are marginalized in (b).
        Notice that marginalization of $\Theta$ introduces dependencies between latent 
        states $y$, since they now share a common parent $\alpha$ in (b).
    }
    \label{fig:hmm-marg}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption[Oscillatory Discrete Time Series]{
        \textbf{Oscillatory Discrete Time Series.}
    }
    \label{fig:osc-step}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption[Inferred States from Trick Coin Example]{
        \textbf{Inferred States from Trick Coin Example.}
        The HMM fit with EM uses a single state to model the observed sequence.
    }
...
\begin{figure}[H]
    ...
    \caption[Discriminative Trick Coin Accuracy]{
        \textbf{Discriminative Trick Coin Accuracy.}
        Whiskers depict the maximum and minimum values.
    }
...
\begin{figure}[H]
    ...
    \caption[Example Segmentation of Synthetic Sequences with Structure]{
        \textbf{Example Segmentation of Synthetic Sequences with Structure.}
    }
...
\begin{figure}[H]
    ...
    \caption[Example Segmentation of Synthetic Sequence without Structure]{
        \textbf{Example Segmentation of Synthetic Sequence without Structure.}
    }
...
\begin{figure}[H]
    ...
    \caption[Synthetic Sequence Clusters]{
        \textbf{Synthetic Sequence Clusters.}
        One sequence from each cluster is shown. 
        The alternating colors depict the recurrent structure
...
\begin{figure}[H]
    ...
    \caption[Synthetic Sequence Clustering Results]{
        \textbf{Synthetic Sequence Clustering Results.}
        Graphs depict the adjusted mutual information and number of discovered
...
\begin{table}[ht]
    ...
    \caption[Summary of Segment Classification Dataset]{
        \textbf{Summary of Segment Classification Dataset.}
    }
...
\begin{table}[ht]
    ...
    \end{tabular}
    \caption[Segmented Exercise Classification Performance]{
        \textbf{Segmented Exercise Classification Performance.}
    }
...
\begin{table}[ht]
    ...
    \end{tabular}
    \caption[Segmented Exercise Classification Performance with \emph{Rest}]{
        \textbf{Segmented Exercise Classification Performance with \emph{Rest}.}
...
\begin{figure}[H]
    ...
    \caption[Unsupervised Exercise Segmentation]{
        \textbf{Unsupervised Exercise Segmentation.}
...
\begin{table}[H]
    ...
    \caption[Exercises Used in Clustering Experiments]{
        \textbf{Exercises Used in Clustering Experiments.}
    }
...
\begin{table}[H]
    ...
    \caption[Unsupervised Clustering Performance]{
        \textbf{Unsupervised Clustering Performance.}
        Comparison of models using \ac{HMM} and \ac{CHMM} base distributions.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption[Relationship between Joint Log Likelihood and AMI]{
        \textbf{Relationship between Joint Log Likelihood and AMI.}
        Three (\subref{fig:cluster-correlation-3}) and nine (\subref{fig:cluster-correlation-9}) 
        exercises were used for various numbers for samples.
...
\begin{figure}[H]
    ...
    \caption[Number of Discovered Clusters]{
        \textbf{Number of Discovered Clusters.}
        DPMoHMM (red) and DPMoCHMM (blue)
        when using three (\subref{fig:cluster-type-counts-3}) and nine (\subref{fig:cluster-type-counts-9})
        different exercises.
    }
...
\begin{figure}[H]
    ...
    \caption[Clustering Confusion Matrices]{
        \textbf{Clustering Confusion Matrices.}
