...
\begin{algorithm}[H]
    \caption{Sample States.\label{alg:har-list}}
    \begin{algorithmic}[1]
        \Require{$\seq{x}{1}{T}$.}
        \State $u_i^1 \gets P(y_i)P(x_1 \mid y_i)$
        \State $p_i^1 \gets u_i^1 (\sum_j u_j^1)^{-1}$
        \For{$t \gets 2, \ldots, T$}
            \State $u_{ij}^t \gets p_i^{t-1}P(j \mid i)P(x_t \mid j)$
            \State $a_{ij}^t \gets u_{ij}^t (\sum_{i^\p j^\p} u_{i^\p j^\p}^t)^{-1}$
            \State $p_j^t \gets \sum_i a_{ij}^t$
        \EndFor
        \State $s^T \sim \Cat(p^T)$
        \For{$t \gets T, \ldots, 2$}
            \State $s^{t-1} \sim \Cat(\sum_i a_{i s^t})$
        \EndFor
    \end{algorithmic}
\end{algorithm}
